...
func ValidatePhoneNumber(phoneNumber string) error {
	...
	r := regexp.MustCompile(`[+][0-9]{3}[(][0-9]{2}[)][0-9]{3}[-][0-9]{2}[-][0-9]{2}`)
	if !r.MatchString(phoneNumber) {
		return errors.New("phone_number must be in format +998(XX)XXX-XX-XX")
	}
	return nil
}
...
func ValidateName(login string) error {
	...
	if validation.Validate(login, validation.Match(regexp.MustCompile("^[A-Za-z]"))) != nil {
		return errors.New("name should contain only alphabetic characters.")
	}
	return nil
}
